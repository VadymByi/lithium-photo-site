...
export default function SliderManagerUI({ allPhotos, initialItems }: Props) {
  ...
  const handleCreate = async (formData: FormData) => {
    ...
    startTransition(async () => {
      // Если выбран режим "выбор из галереи", проверяем наличие photoId
            ...
            >
              ...
              <button
                onClick={() => {
                  if (
                    confirm(
                      'Удалить этот слайд? Фото останется в системе, если оно используется в проектах.',
                    )
                  ) {
